...
def readTexture(n_imgs):
      ...
      img_temp = np.asarray(img_temp_ds.ReadAsArray(0, 0, np.int(img_temp_ds.RasterXSize),np.int(img_temp_ds.RasterYSize)))
      img_temp = np.moveaxis(img_temp, 0, -1)
...
def classify(model_name, texture_flag, n_imgs, glcm, img_path):    
    ...
    patchSizeX = math.floor(xsize)
    patchSizeY = math.floor(ysize)
    patchSizeXorig = math.floor(xsize)
    patchSizeYorig = math.floor(ysize)   
    #mapPredictedFinal = np.zeros((ysize, xsize), dtype=int)
    probMap = np.zeros(((ysize, xsize, len(n_classes))))
    print('got probMap')
   
    for i in range(1):     #IW 5
        for j in range(1): #IW
...
def parallelclassify(model_name, texture_flag, n_imgs, glcm, img_path):    
    ...
    for i in range(5):     #IW 5
        ...
        results = Parallel(n_jobs=5, require='sharedmem')(delayed(processInput)(i,j,minValue,maxValue,patchSizeX,xsize,patchSizeXorig,patchSizeY,ysize,patchSizeYorig,n_imgs,texture_flag,std,gradient,model, glcm, img_path) for j in inputs)      
...
##    img_path = '6.svm-opt-train-glcm-final_test-data/21KXT/'
    model_names = [
...
##        '6.svm-opt-train-glcm-final_test-data/trained_SVM_21KXT_round_std_time-reduced.sav',
        '6.svm-opt-train-glcm-final_test-data/trained_SVM.sav',
...
##        val_name = os.path.basename(validation_path)
        ...
        if(validation_path): # run the code to validate the model
            ...
            probMap, n_classes = parallelclassify(model_name, texture_flag, n_imgs, glcm)
            saveOutputFiles(n_imgs, n_classes, output_path, probMap, tile, pathL, rowL, year, area)
            print('execution_time:',datetime.now() - startTime)
        else:
            probMap, n_classes = classify(model_name, texture_flag, n_imgs, glcm)
            saveOutputFiles(n_imgs, n_classes, output_path, probMap, tile, pathL, rowL, year, area)
            print('execution_time:',datetime.now() - startTime)
